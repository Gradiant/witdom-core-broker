...
BP_TAG="cloudify2"
S1_EXTERNAL_PORT=50100
S1_INTERNAL_PORT=5000
S1_PORT=$S1_INTERNAL_PORT
S1_IMAGE="witdom/broker_td:latest"
...
W2_PORT=8080
W2_IMAGE=$W1_IMAGE
W2_NAME="trusted-service_$BP_TAG"
W2_DESC="Worker2 Description"
W2_IMAGE_URL="https://gradiant.org/wp-content/themes/gradiant/images/Gradiant_logo.jpg"
...
docker run -d -p $S1_EXTERNAL_PORT:$S1_INTERNAL_PORT --add-host=broker:REPLACE_WITH_DOCKER_ENGINE_IP -e BROKER_PORT=50000 --add-host=cloudify:$CLOUDIFY_IP_ADDRESS --link $S2_NAME:mongo --name $S1_NAME $S1_IMAGE
...
#W2_PARAMS="-p $W2_PORT:8080 --add-host=broker:$S1_IP_ADDRESS -e BROKER_PORT=$S1_PORT"
W1_PARAMS="--add-host=broker:$S1_IP_ADDRESS -e BROKER_PORT=$S1_PORT"
W2_PARAMS="--add-host=broker:$S1_IP_ADDRESS -e BROKER_PORT=$S1_PORT"


docker run -d --name $W1_NAME $W1_PARAMS $W1_IMAGE
docker run -d --name $W2_NAME $W2_PARAMS $W2_IMAGE



W1_IP_ADDRESS=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $W1_NAME)
W2_IP_ADDRESS=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $W2_NAME)
...
ctx logger info "W2 Docker container IP address is $W2_IP_ADDRESS"
ctx instance runtime-properties w2_ip_address $W2_IP_ADDRESS
ctx instance runtime-properties w2_internal_port $W2_PORT
ctx instance runtime-properties w2_image_url $W2_IMAGE_URL
ctx instance runtime-properties w2_name $W2_NAME
